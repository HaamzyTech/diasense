...
from app.api.deps import db_session_scope, get_auth_service, resolve_access_token
...
async def signup(payload: SignupRequest) -> SignupResponse:
    with db_session_scope() as db:
        ...
        result = service.signup(payload.username, payload.email, payload.password)
...
async def logout(
    ...
    with db_session_scope() as db:
        service: AuthService = get_auth_service(db)
        token = resolve_access_token(request, payload.access_token if payload else None)
...
async def reset_password(request: Request, payload: ResetPasswordRequest) -> ResetPasswordResponse:
    with db_session_scope() as db:
        ...
        if payload.access_token is not None or request.headers.get("authorization"):
            token = resolve_access_token(request, payload.access_token)
